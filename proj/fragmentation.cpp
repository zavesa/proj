...
void low_level_fragmentation::GetNewBoxes(const Box& box, boxes_pair& new_pair_of_boxes)
{
	...
	if (abs(width) > abs(height)) // abs на вс€кий случай 
...
void high_level_analysis::GetSolution()
{
	...
	std::vector<Box> test; // создаем чистый вектор

	int level = FindTreeDepth();
	for (int i = 0; i < (level + 1); ++i)
	{
		temporary_boxes.move_out(current_boxes); // забираем содержимое из вроеменного вектора
		temporary_boxes.set_value(test); // обнул€ем временный вектор
